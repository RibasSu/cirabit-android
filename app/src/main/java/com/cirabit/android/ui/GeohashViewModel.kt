...
import com.cirabit.android.nostr.GeohashMessageHandler
import com.cirabit.android.nostr.GeohashRepository
import com.cirabit.android.nostr.NostrDirectMessageHandler
import com.cirabit.android.nostr.NostrIdentityBridge
import com.cirabit.android.nostr.NostrProtocol
import com.cirabit.android.nostr.NostrRelayManager
import com.cirabit.android.nostr.NostrSubscriptionManager
...
class GeohashViewModel(
    ...
    private var locationChannelManager: com.cirabit.android.geohash.LocationChannelManager? = null
    ...
    fun initialize() {
        ...
        try {
            locationChannelManager = com.cirabit.android.geohash.LocationChannelManager.getInstance(getApplication())
            ...
            state.setSelectedLocationChannel(com.cirabit.android.geohash.ChannelID.Mesh)
    ...
    fun sendGeohashMessage(content: String, channel: com.cirabit.android.geohash.GeohashChannel, myPeerID: String, nickname: String?) {
        viewModelScope.launch {
            try {
                ...
                val localMsg = com.cirabit.android.model.CirabitMessage(
                    id = tempId,
                ...
                if (startedMining) {
                    com.cirabit.android.ui.PoWMiningTracker.startMiningMessage(tempId)
                }
                try {
                    ...
                    if (startedMining) {
                        com.cirabit.android.ui.PoWMiningTracker.stopMiningMessage(tempId)
                    }
                }
            } catch (e: Exception) {
    ...
    fun startGeohashDM(pubkeyHex: String, onStartPrivateChat: (String) -> Unit) {
        ...
        val gh = (current as? com.cirabit.android.geohash.ChannelID.Location)?.channel?.geohash
        if (!gh.isNullOrEmpty()) {
    ...
    fun startGeohashDMByNickname(nickname: String, onStartPrivateChat: (String) -> Unit) {
        ...
        if (pubkey != null) {
            ...
            Log.w(TAG, "Cannot start geohash DM: nickname '$nickname' not found in repository")
        }
    }
    ...
    fun startGeohashDMByShortId(shortId: String, onStartPrivateChat: (String) -> Unit) {
        ...
        if (pubkey != null) {
            ...
            Log.w(TAG, "Cannot start geohash DM: shortId '$shortId' not found in repository")
        }
    }
    ...
    private fun switchLocationChannel(channel: com.cirabit.android.geohash.ChannelID?) {
