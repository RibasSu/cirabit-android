package com.cirabit.android.ui.debug
...
import com.cirabit.android.services.meshgraph.MeshGraphService
        ...
        if (isPresented) {
            ...
            while (true) {
                ...
                val devices = entries.map { (address, isClient, rssi) ->
                    ...
                    com.cirabit.android.ui.debug.ConnectedDevice(
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        val p = remember(gcsFpr) { com.cirabit.android.sync.GCSFilter.deriveP(gcsFpr / 100.0) }
                        val nmax = remember(gcsFpr, gcsMaxBytes) { com.cirabit.android.sync.GCSFilter.estimateMaxElementsForSize(gcsMaxBytes, p) }
