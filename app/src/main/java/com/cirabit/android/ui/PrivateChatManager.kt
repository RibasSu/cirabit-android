...
class PrivateChatManager(
    ...
    private val unreadReceivedMessages = mutableMapOf<String, MutableList<CirabitMessage>>()
    ...
    fun handleIncomingPrivateMessage(message: CirabitMessage) {
        handleIncomingPrivateMessage(message, suppressUnread = false)
    }

    fun handleIncomingPrivateMessage(message: CirabitMessage, suppressUnread: Boolean) {
    ...
    fun sendReadReceiptsForPeer(peerID: String, meshService: BluetoothMeshService) {
        ...
        val chats = try { state.getPrivateChatsValue() } catch (_: Exception) { emptyMap<String, List<CirabitMessage>>() }
    ...
    private fun consolidateNostrTempConversationIfNeeded(targetPeerID: String) {
        ...
        try {
            ...
            val npub = com.cirabit.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKeyBytes)
            if (npub != null) {
